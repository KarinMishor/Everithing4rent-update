...
public class searchSecondHandController {
    ...
    {
        if(condition.getValue().equals("") || maxPrice_fld.getText().isEmpty() || minPrice_fld.getText().isEmpty()
                || start_date.getValue()==null || end_date.getValue()==null){
            showAlertError("יש להזין את מצב הפריט, מחיר מינימלי, מחיר מקסימלי ותאריכים");
            return;
        }

        try
        {
            String cond=condition.getSelectionModel().getSelectedItem().toString();
            Class.forName("org.sqlite.JDBC");
            Connection conn = DriverManager.getConnection("jdbc:sqlite:db.db");
            //String
            String h=freeText_fld.getText();
            String j=type_fld.getText();
            PreparedStatement prep = conn.prepareStatement("select c.packNum, c.prPolicy, c.cancelPolicy, c.price, c.trading ,a.name, a.description, a.condition, c.startDate, c.endDate from secondHand a join packItems b on a.ID=b.itemnum join packages c on b.packnum=c.packNum where a.condition=? and a.description like '%"+h+"%' and a.name like '%"+j+"%' and c.price>=? and c.price<=? and c.startDate<=? and c.endDate>=?;");
            prep.setString(1,condition.getSelectionModel().getSelectedItem().toString());
